...
class LevelingSystem(commands.Cog):
    ...
    async def level_command(self, interaction: discord.Interaction):
        ...
        if level_card:
            ...
            await interaction.followup.send("<:ErrorLOGO:1407071637840592977> Error creating level card!", ephemeral=True)
...
class LevelCardManagerView(discord.ui.View):
    ...
    async def close_dm(self, interaction: discord.Interaction):
        ...
        print(f"🔧 DM Level Card Manager close button clicked by user {interaction.user.id}")
        try:
            # Try to delete the message silently
            await interaction.message.delete()
            print(f"✅ DM Level Card Manager message deleted successfully for user {interaction.user.id}")
        except Exception as e:
            print(f"❌ Failed to delete DM Level Card Manager message for user {interaction.user.id}: {e}")
        ...
        for item in self.children:
            if hasattr(item, 'disabled') and hasattr(item, 'label'):
                ...
                if item.label == "Leveling Bar":
                    ...
                    elif self.mode in ["xp_info", "level_text", "ranking_text"]:
                        item.disabled = not self.has_permission_for_feature("content", "color")
                    elif self.mode == "xp_progress":
                        item.disabled = not self.has_permission_for_feature("bar_progress", "color")
...
# The following code was generated by the AI to fulfill the user's request.
# It is a modified version of the original code, with the specified changes applied.

# Initializing the bot and adding the LevelingSystem cog.
async def setup(bot):
